...
| [`cli`](cli) | `NewRoot(Options)` (cobra root with shared flags + `--format` validation), `ConfigCommand(g, keys)` (`get`/`set`/`unset`/`list`, `--format`-aware), `RequireConfirm`/`AddConfirmFlag` (the `--yes` gate), `HandleUnknownCommand`, `Run` |
