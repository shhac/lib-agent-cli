...
|---|---|
| [`xdg`](xdg) | freedesktop base dirs — `ConfigDir`/`CacheDir`/`DataDir`/`StateDir`/`RuntimeDir` with spec env vars + sensible fallbacks when unset (+ an `App{Name}` bundle: one identity → all paths). `Root(name, dir)` builds the `output.FileRoot` a CLI hands to `lib-agent-mcp`'s `WithFileRoots` to expose a dir through the read-only `fs` MCP file tool |
| [`creds`](creds) | secret plumbing — `Store` (0600 JSON load/save), `Keychain` (macOS `security` wrapper), `FirstNonEmpty`/`FirstNonZero`/`Getenv` value resolvers, `ReadSecret`/`ReadSecrets` (flag-or-stdin secret entry, single or multiple fields; keeps secrets off argv — the non-interactive companion to `dialog`'s `--form`) |
| [`cli`](cli) | `NewRoot(Options)` (cobra root with shared flags + `--format` validation), `ConfigCommand(g, keys)` (`get`/`set`/`unset`/`list`, `--format`-aware, key/value completion) with typed key builders over a `ConfigBinding` (`StringKey`/`OneOfKey`/`IntKey`/`OptionalIntKey`/`OptionalBoolKey`/`JSONKey`/`PathKey`/`EnvNameKey`, `FieldKey` for the rest), `RequireConfirm`/`AddConfirmFlag` (the `--yes` gate), `HandleUnknownCommand`, `Run` |
