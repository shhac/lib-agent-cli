...
| [`creds`](creds) | secret plumbing — `Store` (0600 JSON load/save), `Keychain` (macOS `security` wrapper), `FirstNonEmpty`/`FirstNonZero`/`Getenv` value resolvers, `ReadSecret`/`ReadSecretLines` (flag-or-stdin secret entry; keeps secrets off argv — the non-interactive companion to `dialog`'s `--form`) |
